...
export const setupAuthServer = ({authServerUrl, mcpServerUrl, strictResource}: {authServerUrl: URL, mcpServerUrl: URL, strictResource: boolean}): OAuthMetadata => {
  ...
  const useGoogleOAuth = process.env.GOOGLE_CLIENT_ID && 
                         process.env.GOOGLE_CLIENT_SECRET && 
                         process.env.JWT_SECRET;
  
  if (useGoogleOAuth) {
    console.log('Using Google OAuth 2.0 authentication');
  } else {
    console.log('Google OAuth credentials not found, using demo in-memory authentication');
  }
  ...
  const provider = useGoogleOAuth
    ...
    : new DemoInMemoryAuthProvider(validateResource);
